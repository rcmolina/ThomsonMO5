...
int main(int argc, char **argv)
{
  ...
  if ((l<1)||((strcmp(&argv[k][l],".k5"))&&(strcmp(&argv[k][l],".K5"))))
  {
  ...
  do
  {
    ...
    if (c==60)
    {
      ...
      if (c==90)
      {
        ...
        c=myfgetc(fp,fw);
        writebyte(fw,c);
        if (((c+s)&255) && (b!=0xff)) printf("erreur checksum!? sum=0x%02X should(256-sum)=0x%02X found=0x%02X\n", s&255, 256-(s&255), c);
      } else printf("erreur\n");
    }
    else
    {
